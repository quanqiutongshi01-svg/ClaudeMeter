...
> Menu bar examples: `Cl 94% 120ms · Cx 92% 180ms` or `Cl 94%/85% 120ms · Cx 92%/96% 180ms`
...
- **Server latency.** Shows lightweight HTTPS latency to `api.anthropic.com` and `chatgpt.com/backend-api/` without sending model requests.
- **Latency toggle.** Turn latency display on or off from the floating panel; the app remembers your choice and skips probes when disabled.
...
> 菜单栏示例：`Cl 94% 120ms · Cx 92% 180ms` 或 `Cl 94%/85% 120ms · Cx 92%/96% 180ms`
...
- **服务器延迟。** 显示到 `api.anthropic.com` 和 `chatgpt.com/backend-api/` 的轻量 HTTPS 延迟，不发送模型请求。
- **延迟开关。** 可在浮窗中打开或关闭延迟显示；App 会记住你的选择，并在关闭时跳过延迟探测。

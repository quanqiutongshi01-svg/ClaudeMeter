...
No Electron, no daemon, no hidden Codex request. The menu bar stays compact, and the floating panel shows the full Claude/Codex breakdown.

![TokenMeter floating panel](assets/screenshot.png)

> Menu bar example: `Cl 9% · ~Cx 59%`
> `~Cx` means the latest Codex snapshot may be stale.
...
非 Electron、无后台守护进程、不会偷偷发 Codex 请求。菜单栏保持紧凑，浮窗里展示 Claude/Codex 的完整拆分。

> 菜单栏示例：`Cl 9% · ~Cx 59%`
> `~Cx` 表示最新 Codex 快照可能已经过期。
